...
$(document).ready(function () {
        ...
        s = parseInt("<?php echo $OFFERS['BONDS_PER_PAGE']; ?>", 10),
        c = parseInt("<?php echo $OFFERS['FUNDS_PER_PAGE']; ?>", 11),
        ...
        function anleihenz() {
            ...
            a = u.data("user");
            $.ajax({
                url: "<?php echo $BASE; ?>/api/data/offersanleihen" + (a ? "/" + a : ""),
                method: "GET",
                dataType: "json",
                async: !0,
                cache: !1,
                success: function (a) {
                    if (q.destroyDataTable(), q.find("tbody").empty(), a && a.length) {
                        ...
                        $.each(a, function (a, e) {
                            // last column
                            if(e.interest_date){
                                var c = "<h3 class='ui dividing header centered'><?php echo $offers['anleihen']['interest_date']; ?></h3><div class='content'>", 
                                    j = e.interest_date;
                                if(j){
                                    var o = JSON.parse(j);
                                    $.each(o, function(i,v){
                                ...
                                c += "</div>";
                                d = '<button title="<?php echo $offers['anleihen']['interest_tooltip']; ?>" class="interest-date ui icon <?php echo $SITE['MAIN_COLOR']; ?> button" data-html="'+c+'"><i class="table icon"></i></button>';
                            } else {
                            ...
                            if(e.notes && e.notes != ''){
                                ...
                                fa = '<button title="<?php echo $offers['anleihen']['notes_tooltip']; ?>" class="anleihen-notes ui icon <?php echo $SITE['MAIN_COLOR']; ?> button" data-notes="' + w + '"><i class="info icon"></i></button>';
                            } else {
        ...
        function fundsff() {
            ...
            a = u.data("user");
            $.ajax({
                url: "<?php echo $BASE; ?>/api/data/offersfunds" + (a ? "/" + a : ""),
                method: "GET",
                dataType: "json",
                async: !0,
                cache: !1,
                success: function (a) {
                    if (fl.destroyDataTable(), fl.find("tbody").empty(), a && a.length) {
                        ...
                        $.each(a, function (a, e) {
                            ...
                            t += 
                            "<tr class='positive'>"+
                            "<td><div class='row'>" + e.name + '</div><br><div class="ui labeled button" tabindex="0"><div class="ui basic <?php echo $SITE['MAIN_COLOR']; ?> button">BANK</div><a class="ui basic left pointing blue label">' + e.symbol + '</a></div><br><br><div class="row trade-exchange"><i class="' + e.exchange_country_code + ' flag"></i>' + e.exchange_name + "</div></td>" +
