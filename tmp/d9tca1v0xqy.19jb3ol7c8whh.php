...
$(document).ready(function () {
    ...
    function v() {
        var a = s.data("user");
        $.ajax({
            url: "<?php echo $BASE; ?>/api/data/depot/history" + (a ? "/" + a : ""),
            method: "GET",
            dataType: "json",
            async: !0,
            cache: !1,
            success: function (a) {
                ...
                l.initDataTable(10, [[7, "desc"],[4, "desc"],[5,"desc"]], !0, null, [$.extend(!0, {}, {
                        extend: "print",
                        autoPrint: !1,
                        title: "<?php echo $depot['transactions']['printfor_title']; ?>" + (s.find('div.title').text() ? s.find('div.title').text() : '<?php echo $USER['first_name']." ".$USER['last_name']; ?>'),
                        text: "<i class='print icon'></i> <?php echo $portfolio['positions']['print']; ?>",
                        exportOptions: {
    ...
    function u() {
        $.ajax({
            url: "<?php echo $BASE; ?>/api/data/depot/history",
            method: "GET",
            dataType: "json",
            async: !0,
            cache: !1,
            success: function (a) {
                if (log("displayTransactionsHistory", a), l.destroyDataTable(), l.find("tbody").empty(), a && a.length) {
                    var t = "";
                    $.each(a, function (a, e) {
                        t += '<tr class="' + (e.processed ? 'positive':'negative') + '"><td class="center aligned">' + (e.processed ? '<i class="check circle outline icon" title="'+moment(e.processed).format("DD MMMM YYYY HH:mm:ss")+'"></i>' : '<i class="hourglass half icon" title="<?php echo $depot['transactions']['waiting']; ?>"></i>')  + '</td><td>' + e.first_name + ' ' + e.last_name + '</td><td>' + (e.type == 0?'<?php echo $depot['transactions']['withdraw']; ?>':'<?php echo $depot['transactions']['deposit']; ?>') + "</td><td>" + floatVal(e.amount).formatNumber() +  "</td><td class='right aligned'>" + moment(e.date).format('DD MMMM YYYY') + '</td><td class="left aligned">' + e.time + '</td><td class="center aligned"><a href="' + 'files/docs/' + e.doc_path + '" target="_blank"><i class="download icon"></i></a></td><td class="center aligned">' + (e.processed ? moment(e.processed).format('DD MMMM YYYY HH:mm') : '<?php echo $depot['transactions']['waiting']; ?>')  + '</td><td class="depot-actions right aligned"><div class="button-container"><button class="ui basic circular icon edit-transaction-button button" data-depot-id="' + e.t_id.replace('id','') + '" data-type="' + e.type + '" data-user-id="' + e.user_id + '" data-amount="' + e.amount + '" data-date="' + e.date + '" data-time="' + e.time + '" data-docpath="' + e.doc_path + '" data-notes="' + e.notes + '" data-status="' + e.status + '"><i class="pencil icon"></i></button></div><div class="button-container"><button class="ui basic circular icon delete-transaction-button button" data-depot-id="' + e.t_id.replace('id','') + '"><i class="trash icon"></i></button></div></td></tr>'
                    }),
                    l.find("tbody").append(t)
                }
                l.initDataTable(10, [[7, "desc"],[4, "desc"],[5,"desc"]], !0, null, [$.extend(!0, {}, {
                        extend: "print",
                        autoPrint: !1,
                        title: "<?php echo $depot['transactions']['printall_title']; ?>",
                        text: "<i class='print icon'></i> <?php echo $portfolio['positions']['print']; ?>",
                        exportOptions: {
                            columns: [":not(:nth-child(1)):not(:nth-child(7))"]
                        }
    ...
    function c(){
        $.ajax({
            url: "<?php echo $BASE; ?>/api/data/depot/upcount",
            method: "GET",
            dataType: "json",
            async: !0,
            cache: !1,
    ...
    $("#modal-depot-delete").form({
        ...
        if (!e.hasClass("submitting")) {
            ...
            $.ajax({
                url: "<?php echo $BASE; ?>/api/actions/depot/delete",
                method: "POST",
                data: f,
                dataType: "json",
                async: !0,
    ...
    $(".revert.button").click(function(e){
        ...
        if (!e.hasClass("submitting")) {
            ...
            r.hide(),
            $.ajax({
                url: "<?php echo $BASE; ?>/api/actions/depot/update",
                method: "POST",
                data: f,
                dataType: "json",
                async: !0,
                ...
                success: function (a) {
                    ...
                    $(".submit.button").removeClass("disabled loading"),
                    a && a.success ? ($(".submit.button").html("<?php echo $depot['transactions']['save']; ?> <i class='checkmark icon'></i>").removeClass("disabled"), $(".revert.button").hide()) : ($(".submit.button").html("<?php echo $depot['transactions']['approved']; ?> <i class='check circle outline icon'></i>").addClass("disabled"), $(".revert.button").show()),
                    setTimeout(function () {
                        r.hide("slow")
                    }, 3e3),
                    e.removeClass("submitting"),
    ...
    l.on("click", ".edit-transaction-button", function () {
        ...
        a.data("status") && a.data("status") == 1 ? ($(".submit.button").html("<?php echo $depot['transactions']['approved']; ?> <i class='check circle outline icon'></i>").addClass("disabled"), $(".revert.button").show()) : ($(".submit.button").html("<?php echo $depot['transactions']['save']; ?> <i class='checkmark icon'></i>").removeClass("disabled"), $(".revert.button").hide());
    });
        ...
        if (!e.hasClass("submitting")) {
            ...
            $.ajax({
                ...
                success: function (a) {
                    a && a.success ? r.text(a.message).removeClass("negative").addClass("positive").show() : r.text(a.message).removeClass("positive").addClass("negative").show(),
                    $(".submit.button").removeClass("disabled loading"),
                    a && a.success ? ($(".submit.button").html("<?php echo $depot['transactions']['approved']; ?> <i class='check circle outline icon'></i>").addClass("disabled"), $(".revert.button").show()) : ($(".submit.button").html("<?php echo $depot['transactions']['save']; ?> <i class='checkmark icon'></i>").removeClass("disabled"), $(".revert.button").hide()),
                    setTimeout(function () {
                        r.hide("slow")
                    }, 3e3),
                    e.removeClass("submitting"),
...
$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (a) {
                if (log("displayTransactionsHistory", a), l.destroyDataTable(), l.find("tbody").empty(), a && a.length) {
                    var t = "";
                    $.each(a, function (a, e) {
                        t += '<tr class="' + (e.processed ? 'positive':'negative') + '"><td class="center aligned">' + (e.processed ? '<i class="check circle outline icon" title="'+moment(e.processed).format("DD MMMM YYYY HH:mm:ss")+'"></i>' : '<i class="hourglass half icon" title="Waiting"></i>')  + '</td><td>' + (e.type == 0?'<?php echo $depot['transactions']['withdraw']; ?>':'<?php echo $depot['transactions']['deposit']; ?>') + "</td><td>" + floatVal(e.amount).formatNumber() +  "</td><td class='right aligned'>" + moment(e.date).format('DD MMMM YYYY') + '</td><td class="left aligned">' + e.time + '</td><td class="center aligned"><a href="' + 'files/docs/' + e.doc_path + '" target="_blank">view</a></td><td class="center aligned">' + (e.processed ? moment(e.processed).format('DD MMMM YYYY HH:mm') : '<?php echo $depot['transactions']['waiting']; ?>')  + '</td></tr>'
                    }),
                    l.find("tbody").append(t)
                }
                l.initDataTable(10, [[6,"desc"],[3, "desc"],[4,"desc"]], !0, null, [$.extend(!0, {}, {
                        extend: "print",
                        autoPrint: !1,
                        title: "<?php echo $depot['transactions']['printfor_title']; ?>" + "<?php echo $USER['first_name'].' '.$USER['last_name']; ?>",
                        text: "<i class='print icon'></i> <?php echo $portfolio['positions']['print']; ?>",
                        exportOptions: {
    ...
    $("#modal-depot").form({
        ...
        if (e.form("is valid") && !e.hasClass("submitting")) {
            ...
            r.hide(),
            $.ajax({
                url: "<?php echo $BASE; ?>/api/actions/depot/save",
                method: "POST",
                data: d,
                dataType: "json",
                contentType: !1,
