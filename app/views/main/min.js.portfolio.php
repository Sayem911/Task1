...
$(document).ready(function () {
    ...
    function zamanlayici() {    
      ...
      console.log(time);
        ...
        s = parseInt("{{ @PORTFOLIO.TRADES_PER_PAGE }}", 10),
        c = parseInt("{{ @PORTFOLIO.POSITIONS_PER_PAGE }}", 10),
        u = "{{ @USER.currency }}",
        ...
        function anleihenlistz() {
            ...
            a = u.data("user");
            $.ajax({
                url: "{{ @BASE }}/api/data/anleihen" + (a ? "/" + a : ""),
                method: "GET",
                dataType: "json",
                async: !0,
                cache: !1,
                success: function (a) {
                    if (q.destroyDataTable(), q.find("tbody").empty(), a && a.length) {
                        ...
                        $.each(a, function (a, e) {
                            if(e.interest_date){
                                var c = "<h3 class='ui dividing header centered'>{{ @portfolio.anleihen.interest_date }}</h3><div class='content'>", 
                                    j = e.interest_date;
                                if(j){
                                    var o = JSON.parse(j);
                                    $.each(o, function(i,v){
                                ...
                                c += "</div>";
                                d = '<button title="{{ @portfolio.anleihen.interest_tooltip }}" class="interest-date ui icon {{ @SITE.MAIN_COLOR}} button" data-html="'+c+'"><i class="table icon"></i></button>';
                            } else {
                                d = '<button class="ui icon {{ @SITE.MAIN_COLOR}} button button disabled"><i class="table icon"></i></button>';
                            }
                            if(e.notes && e.notes != ''){
                                var w = e.notes.toString().replace("\n", "<br />");
                                fa = '<button title="{{ @portfolio.anleihen.notes_tooltip }}" class="anleihen-notes ui icon {{ @SITE.MAIN_COLOR}} button" data-notes="' + w + '"><i class="info icon"></i></button>';
                            } else {
            ...
            $.ajax({
                ...
                success: function (a) {
                    if (fonds.destroyDataTable(), fonds.find("tbody").empty(), a && a.length) {
                        ...
                        $.each(a, function (a, e) {
                            if(e.interest_date){
                                var c = "<h3 class='ui dividing header centered'>{{ @portfolio.fonds.interest_date }}</h3><div class='content'>", 
                                    j = e.interest_date;
                                if(j){
                                    var o = JSON.parse(j);
                                    $.each(o, function(i,v){
                                        c += "<div><span class='ui {{ @SITE.MAIN_COLOR}} tiny circular label'>" + (i+1) + "</span> &nbsp;&nbsp; " + moment(v.date).format('DD.MM.YYYY') + "&nbsp;&nbsp;&nbsp;&nbsp;" + floatVal(v.current_value).formatNumber(2) + " {{ @USER.currency}} </div><div class='ui divider'></div>";
                                    });
                                }
                                c += "</div>";
                                d = '<button title="{{ @portfolio.fonds.interest_tooltip }}" class="interest-date ui icon {{ @SITE.MAIN_COLOR}} button" data-html="'+c+'"><i class="table icon"></i></button>';
                            } else {
                                d = '<button class="ui icon {{ @SITE.MAIN_COLOR}} button button disabled"><i class="table icon"></i></button>';
                            }
                            if(e.notes && e.notes != ''){
                                var w = e.notes.toString().replace("\n", "<br />");
                                fa = '<button title="{{ @portfolio.fonds.notes_tooltip }}" class="fonds-notes ui icon {{ @SITE.MAIN_COLOR}} button" data-notes="' + w + '"><i class="info icon"></i></button>';
                            } else {
                                fa = '<div class="six wide column"></div>'
                            }
                            t += 
                            ...
                            "<div class='row'>" + e.name + "</div>"+
                            "<div class='row'>"+
                            "<br><div class='ui labeled button' tabindex='0'><div class='ui basic {{ @SITE.MAIN_COLOR}} button'>WKN</div><a class='ui basic left pointing blue label'>" + e.symbol + "</a></div><br><br>"+
                            "<div class='row trade-exchange'><i class='" + e.exchange_country_code + " flag'></i>" + e.exchange_name + "</div>"+
                            "</td>" + 
                ...
                success: function (a) {
                    if (fl.destroyDataTable(), fl.find("tbody").empty(), a && a.length) {
                        ...
                        $.each(a, function (a, e) {
                            ...
                            t += 
                            "<tr class='positive'>"+
                            "<td><div class='row'>" + e.name + '</div><br><div class="ui labeled button" tabindex="0"><div class="ui basic {{ @SITE.MAIN_COLOR}} button">BANK</div><a class="ui basic left pointing blue label">' + e.symbol + '</a></div><br><br><div class="row trade-exchange"><i class="' + 'eu'/*e.exchange_country_code*/ + ' flag"></i>' + 'Festgeld Basis EU+'/*e.exchange_name*/ + "</div></td>" +
                ...
                success: function (a) {
                    if (dl.destroyDataTable(), dl.find("tbody").empty(), a && a.length) {
                        ...
                        $.each(a, function (a, e) {
                            ...
                            t += 
                            "<tr class='positive'>"+
                            "<td><div class='row'>" + e.name + '</div><br><div class="ui labeled button" tabindex="0"><div class="ui basic {{ @SITE.MAIN_COLOR}} button">SHARES (PRE-IPO)</div><a class="ui basic left pointing blue label">' + e.symbol + '</a></div><br><br><div class="row trade-exchange"><i class="' + e.exchange_country_code + ' flag"></i>' + e.exchange_name + "</div></td>" +
        ...
        fonds.on("click", ".edit-fonds-button", function () {
            ...
            y = t.find(".ui.dropdown");
            $.ajax({
                url: "{{ @BASE }}/api/data/fonds/assets/all",
                method: "GET",
                dataType: "json",
                async: !0,
                cache: !1,
                success: function (d) {
                    ...
                    y.dropdown({
                        'values': w, 
                        'placeholder': '{{ @portfolio.fonds.select }}'
                    });
                    y.dropdown('set selected', a.data("symbol"));
        ...
        $("#modal-edit-fonds").form({
            ...
            if (e.form("is valid") && !e.hasClass("submitting")) {
                ...
                i['notes'] = e.find("textarea").val();
                $.ajax({
                    url: "{{ @BASE }}/api/actions/fonds/edit",
                    method: "POST",
                    data: i,
                    dataType: "json",
                    async: !0,
        ...
        $("#modal-add-fonds").form({
            ...
            if (!e.hasClass("submitting")) {
                ...
                i['notes'] = e.find("textarea").val();
                $.ajax({
                    url: "{{ @BASE }}/api/actions/fonds/add",
                    method: "POST",
                    data: i,
                    dataType: "json",
                    async: !0,
        ...
        $("#modal-delete-fonds").form().on("submit", function (a) {
            ...
            if (e.form("is valid") && !e.hasClass("submitting")) {
                ...
                $.ajax({
                    url: "{{ @BASE }}/api/actions/fonds/delete",
                    method: "POST",
                    data: i,
                    dataType: "json",
                    async: !0,
